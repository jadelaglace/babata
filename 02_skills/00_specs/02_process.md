...
- Agent guidance skill (parallel): `02_skills/babata-clean/`
...
- Permissions: invoke Rust CLI only for formal registration; adapters cannot write SQLite or managed
  assets; no provider credentials in Skill
...
- Activation:
  ...
  - P5+: provider-neutral Agent routing may use local, QianWen, Bailian, or later adapters and then
    `agent_import`; adapter-specific OCR/ASR/visual queue providers remain unavailable until implemented
    and evidenced
