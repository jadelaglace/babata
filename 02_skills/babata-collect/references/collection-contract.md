...
Collection distinguishes sovereignty recovery from formal registration. C0-A1/C0-A2 acquisition may end
...
The source-specific acquisition boundary owns C0-A1/C0-A2 retrieval and may stop with an honest
...
| --- | --- |
| `C0-A1` | Preserve immediately available bytes, responses, exports, source identity, hashes, and limitations without waiting for parsing, recursion, or registration. |
| `C0-A2` | Retrieve the body, embedded media, and required attachments needed to make the C0-A1 material locally complete; record each unresolved dependency. |
| `C0-A3+` | Follow a semantic reference only when explicitly valuable and authorised. Save the result as a separate node/reference and stop recursion by default. |

Body text, embedded media, and required attachments are completeness dependencies and normally
belong to C0-A2. A cited article, paper, or page is normally a reference, not a C0-A2 dependency.
...
| `prepared` / `C0-B` | After C0-A2 is complete, non-semantic extraction, stable naming, format identification, manifest, and field preservation are complete without overwriting upstream originals. |
| `registered` / `C0-C` | After C0-A2 and preparation, the unique Rust application/core assigned SQL identity, revision, assets, provenance, relations, status, and successful read-back. |

Depth and readiness are reported independently, but readiness has an admission rule: C0-A1 may only be
`captured`; C0-A2 is required before `prepared / C0-B` or `registered / C0-C`. C0-A3+ is optional and is not
an admission requirement. Only `registered / C0-C` may be
reported as "formally collected", "entered C0", or "formal C0". C0-A1/C0-A2 captured or prepared material
...
| Agent/browser/CLI download in temporary or Recovery storage | C0-A1/C0-A2 captured or prepared | No, but it may count as successful sovereignty recovery |
...
C1/C2 may identify a missing reference and request a new authorised C0-A3/C0-A4 acquisition. They must not
...
1. **拿回**: what the Agent or source tool retrieved and verified, including C0-A1/C0-A2/C0-A3+ depth and
