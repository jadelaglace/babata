...
try{
    ...
    $support=@();for($i=0;$i -lt 4;$i++){$support+=[ordered]@{slot=@('decision_tools','case_practice','review_self_test','evidence_index')[$i];id="aid$($i+1)";note=$aids[$i];legacy_note=$legacyAids[$i]}}
    $presentationMap=$plan.course_map|ConvertTo-Json -Depth 20|ConvertFrom-Json;for($i=0;$i -lt 4;$i++){$presentationMap.learning.nodes[$i].note=$aids[$i]}
    $units=@($chapters|ForEach-Object{[ordered]@{id=$_.id;note=$_.note;title=$_.title;source_modules=@($_.modules|ForEach-Object{[string]$_})}})
    $presentation=[ordered]@{schema='babata.mba-course-presentation-plan/v2';course=$plan.course;short_name=$plan.short_name;course_key=$plan.course_key;expected_modules=5;profile='semantic-obsidian/v2';output_status='pending_user_acceptance';source=[ordered]@{plan_path=$planPath;plan_sha256=$planSha;manifest_path=$sourceMapPath;manifest_sha256=$sourceMapSha};outline=[ordered]@{mode='sectioned';sections=@([ordered]@{id='s1';title='基础';units=@($units[0..1])},[ordered]@{id='s2';title='应用';units=@($units[2..4])})};learning_support=$support;rename_map=[ordered]@{$legacyAids[0]=$aids[0];$legacyAids[1]=$aids[1];$legacyAids[2]=$aids[2]};course_map=$presentationMap;live=$plan.live};Write-Json $presentationPath $presentation 30;$presentationSha=Hash $presentationPath
    ...
    $learning=[ordered]@{schema='babata.mba-course-learning-docs/v1';course='Test MBA Course';status='candidate';course_plan_sha256=$planSha;presentation_plan_sha256=$presentationSha;template_profile='semantic-obsidian/v2';source_map=$sourceMapPath;source_map_sha256=$sourceMapSha;expected_modules=5;complete_source_notes=5;source_notes=$sourceNotes;chapter_documents=5;learning_documents=$docNames.Count;generated_files=$learningRows};Write-Json $learningPath $learning 20
    $stage=Join-Path $runtime 'valid';$result=@(&$materializer -CoursePlanPath $planPath -PresentationPlanPath $presentationPath -LearningDocsManifestPath $learningPath -C1BRegistrationLedgerPath $c1bPath -KnowledgeUniverseLedgerPath $knowledgePath -StagingRoot $stage -DataHome $data)
    ...
    if((Get-Content -LiteralPath (Join-Path $stage 'package\index.md') -Raw -Encoding utf8) -notmatch '(?s)### 基础.*\[\[01-章节1\]\].*### 应用.*\[\[03-章节3\]\]'){throw 'Sectioned presentation order was not materialized'}
    ...
    $checked=@(&$checker -CoursePlanPath $planPath -PresentationPlanPath $presentationPath -PackageRoot (Join-Path $stage 'package') -ManifestPath $manifestPath);if($checked.Count -ne 1 -or $checked[0].status -ne 'passed'){throw 'Materialized mock package did not pass checker'}
    ...
    $zeroMediaStage=Join-Path $runtime 'valid-zero-media';$zeroMediaResult=@(&$materializer -CoursePlanPath $planPath -PresentationPlanPath $presentationPath -LearningDocsManifestPath $learningPath -C1BRegistrationLedgerPath $c1bPath -KnowledgeUniverseLedgerPath $knowledgePath -StagingRoot $zeroMediaStage -DataHome $data)
    ...
    $badStage=Join-Path $runtime 'bad-duplicate-module';Assert-Throws {&$materializer -CoursePlanPath $planPath -PresentationPlanPath $presentationPath -LearningDocsManifestPath $learningPath -C1BRegistrationLedgerPath $c1bPath -KnowledgeUniverseLedgerPath $knowledgePath -StagingRoot $badStage -DataHome $data} 'duplicate knowledge module'
