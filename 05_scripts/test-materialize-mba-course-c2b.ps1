...
try{
    ...
    $aids=@('09-公式与决策工具','10-案例练习','11-复习与自测','视觉证据索引');$aidNodes=@();for($i=0;$i -lt 4;$i++){$aidNodes+=[ordered]@{id="aid$($i+1)";note=$aids[$i]}}
    ...
    $learningRows=@();$docNames=@('00-课程总览')+@($chapters.note)+@('09-公式与决策工具','10-案例练习','11-复习与自测')
