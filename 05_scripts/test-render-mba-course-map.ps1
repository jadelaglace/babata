...
try{
    ...
    $colors=@('#2563EB','#16A34A','#EA8A00','#EF4444','#8B5CF6');$domains=@()
    for($i=1;$i -le 5;$i++){
    ...
    if($result.Count -ne 1 -or $result[0].status -ne 'passed' -or [int]$result[0].mece_domains -ne 5 -or [int]$result[0].internal_link_targets -ne 9){throw 'Valid dynamic course map did not pass'}
    ...
    $spec.domains[1].id='domain1';$spec|ConvertTo-Json -Depth 20|Set-Content -LiteralPath (Join-Path $media 'course-map.spec.json') -Encoding utf8
