...
正常收集应能够从用户已经登录、阅读、收藏或整理资料的真实浏览器上下文开始。优先
复用官方 API/CLI；对大量网页登录态来源，先调查并实际验证 GitHub 上成熟的通用
Agent 浏览器 CLI、Skill、MCP、扩展或 CDP 工具，优先评估 `browser-use` 和
`agent-browser`，让现有 Agent 自主完成导航、翻页、候选发现、结构化读取、网络观察、
下载和重试。站点专用工具负责通用浏览器层缺少的稳定命令、导出或媒体取得能力；没有
合适路径时才增加窄适配能力。
