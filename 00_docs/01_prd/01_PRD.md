...
正常交接应是 Agent 发起安装、登录或 OAuth 流程，用户只在来源官方页面完成登录、
扫码或授权确认，并选择要收集的范围；之后的命令、分页、下载、重试、临时文件、
格式接入和状态由 Agent/Babata 完成。产品不得把手抄密码、Cookie、token、导出路径
或来源元数据设计成日常收集步骤。确实只能接收会话秘密的现有工具，必须先证明没有
更安全路线，并把秘密限制在本机受控存储中。
