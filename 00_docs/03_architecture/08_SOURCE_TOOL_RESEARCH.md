...
| Kimi 对话 | 组合路线待实证 | 通用 Agent 浏览器遍历完整历史；OpenCLI `history/detail/read` 作为第二层 | Chrome 已登录；给出会话或时间范围 | Agent Browser/OpenCLI E1；真实来源 E0 |
...
| Kimi | Chrome 已登录；首次批准 remote debugging | 会话、时间或数量范围 | 自主列历史、读取长对话和附件；OpenCLI 补稳定 ID | 通用浏览器与附件/深研产物探针 |
...
| `agent-browser 0.32.1` | 已全局安装并实际运行 version/help/doctor；doctor 为 7 pass、0 warn、0 fail，headless launch 通过 | 真实连接前在 Chrome 原生页面一次批准 remote debugging |
| `browser-use 0.13.6` | 官方 CLI/Skill、真实 Chrome/CDP 和当前发行已核；本机 `uv tool` 安装因依赖下载长时间停滞而中止并清理残留，尚不算 E1 | Agent 后续修复安装并先跑 doctor；真实连接前一次批准 remote debugging |
...
本机已核官方仓库、当前发行、CLI Skill 和 Browser Harness 连接说明；`uv tool` 安装因
依赖下载长时间停滞而中止，残留环境已清理。未运行 `browser-use --doctor` 前记为 E0，
不能只凭 105k stars 宣称可用。Python 只作为成熟外部 CLI 的受控边界，输出进入 staging，
不得写核心数据。
...
因此真实登录 profile 的首次探针只能在用户一次批准后进行，先限制动作策略为导航、读取、
滚动和候选发现，下载另行按明确收集范围允许；完成后关闭远程调试。

### 5.3 当前选择

按用户明确纠正，真实浏览器探针顺序为：**Browser Use CLI 第一，Agent Browser 第二**；
二者都是主线候选，最终主次由同一真实来源上的 E2/E3 证据决定，不由 star 数决定。
Microsoft `playwright-cli` 作为官方比较组保留；它已达到本机 E1，但不再在未比较这两个
候选前直接定为主线。

OpenCLI 不再承担唯一通用浏览器主线。它保留为第二层：有确定站点命令时列结构化候选、
读详情或下载，补充通用浏览器导航层的稳定性和媒体/导出能力。
...
逐站候选和专用工具已经调查；网页登录来源的通用浏览器主次以及抖音路线仍需真实 Chrome
实证。下一步按价值和最小新增权限验证：

1. 由 Agent 修复 Browser Use CLI 本机安装并运行 `--doctor`；之后用户一次批准 Chrome 当前实例 remote
   debugging 后，Agent 先只列 tab/status，不读取随机私人内容；
2. 在用户给出的一个已登录站点和明确范围上，先用 Browser Use、再用 Agent Browser 各自
   自主列一页候选，比较连续导航、字段、下载、网络观察、恢复、输出和权限；过程中不让
   用户逐条代操作；
3. 选定通用浏览器主线后，依次对 Bilibili、知乎、小红书、豆包、Kimi、ChatGPT 验证一个
   明确收藏夹/会话/时间范围；只有通用层缺稳定结构或媒体时才调用 OpenCLI/`yt-dlp`；
4. 抖音先用选定的通用 Agent 浏览器遍历已登录收藏页；所选媒体再验证 `F2`，若仍失败才
   立项窄浏览器候选适配器；
5. P4 浏览器扩展直接调用 `chrome.bookmarks`、activeTab 和 SingleFile，证明页面/选区/
   书签候选与选择性提交；
6. 语雀扩展验证知识库列表、目录、附件和一次重复导出；
7. OneNote 用 `Notes.Read` 验证一个 notebook/section/page/resource；Evernote 在用户明确
   账号级回收后验证一次 sync 和第二次增量 sync；
8. 微信按“公众号链接 -> 收藏 -> 视频号 -> 聊天”的风险顺序验证，数据库读取路线最后
   且必须再次明确确认；
9. 每条来源完成 E3 前保持 disabled，不用 fixture、命令 help 或下载文件数量冒充支持。

## 14. P2-G7 当前判断

所有 00 已点名来源都已有第一轮逐站调查；第三轮又补上 Browser Use、Agent Browser、
Playwright CLI 与 OpenCLI 的通用 Agent 浏览器方向。除飞书官方 CLI 外，网页登录来源的
最终主次路线仍需同一真实 Chrome 上的比较实证；这比把 OpenCLI 或某个下载器提前写成
唯一主线更诚实。抖音旧首选经第二轮调查被证伪，现已改为先验证通用浏览器导航，再验证
媒体工具的明确缺口。

但 P2-G7 原文还要求“实际验证”。当前只有飞书达到 E2；Agent Browser、Playwright、
OpenCLI 和部分专用工具达到 E1，Browser Use CLI 本机安装仍待完成，抖音、浏览器扩展、
OneNote 和微信窄适配仍为 E0/E1；尚未完成“给出一次范围 -> Agent 自主发现/翻页/下载 ->
内容/附件 -> 重收集”的 E3 闭环。因此 P2-G7 **部分通过，P2 继续进行中**。下一步不是
继续堆 adapter 空文件，而是按第 13 节做一次真实 Chrome 小范围自主收集探针。
