...
| source.onenote | OneNote | 官方桌面客户端整本导出 PDF + MHT，Agent 跟随一次明确笔记本范围取得导出件 | 客户端已登录；选择一个或多个笔记本范围 | E0：用户已确认整本 PDF/MHT 官方导出可用；未执行真实导出 | 缺一次真实整本导出、C0、失败状态和重采；不再继续 CLI/API 调研 | disabled |
| source.evernote | 印象笔记 / Evernote | 官方客户端按明确笔记本/账号范围导出 HTML 及资源目录 | 客户端已登录；选择笔记本或账号范围 | E0：用户已确认 HTML 官方导出可用；未执行真实导出 | 缺一次真实 HTML/资源导出、C0、失败状态和重采；不再继续 CLI/API 调研 | disabled |
...
| source.wechat_channels | 微信视频号 | 收藏 UI 发现候选，选中后用 `res-downloader` 捕获原媒体 | 已登录微信；显式批准临时本地代理证书并播放所选项 | E1：候选工具和权限模型已核；2026-07-19 用户决定暂时不处理 | 延期到用户重新启用；缺高风险工具单独确认、真实媒体样本和 E3 | disabled |
...
| 微信视频号 | 当前无需动作 | 暂无 | 保留历史路线，不安装代理证书或捕获工具 | 用户已决定暂时不处理 |
...
决策：**官方桌面整本 PDF + MHT 导出；停止继续调研**。当前尚未执行真实样本，保持
`disabled`，转入 P7 扩展来源而不是阻塞 P4。
...
正常执行是：用户只选择一次笔记本或账号范围，Agent 跟随官方客户端导出 HTML 和随附
资源目录，保存目录层级、页面文件、图片/附件、导出时间、客户端版本、文件大小和
SHA-256，再通过 Babata 唯一 C0 路径登记。缺失的任务、提醒、内部链接、历史版本或
客户端视觉状态必须作为导出限制记录，不能猜测补齐。

决策：**官方 HTML 导出；停止继续调研**。当前尚未执行真实样本，保持 `disabled`，转入
P7 扩展来源而不是阻塞 P4。
...
决策：**暂时延期并保持 disabled；用户重新启用后再评估现成媒体工具 + 窄候选适配**。
