...
| --- | --- | --- | --- | --- | --- | --- |
| source.feishu | 飞书文档、Wiki、知识库、云文档 | 官方 `lark-cli` 直接调用，Babata 只包授权、范围选择和结果接入 | 一次飞书应用配置与用户 OAuth；以后选择文档/节点/范围 | E2：真实用户授权和 Wiki/Docs 只读调用成功 | 缺一次正文加附件的 E3 样本、逐条状态和重收集 | disabled |
| source.yuque | 语雀 | 已登录浏览器中的批量导出扩展；Agent 批处理可用 `yuque-dl` | 登录语雀并选择知识库/文档；CLI 路线另需受控本机会话 | E1：`yuque-dl` 已安装并核命令 | 缺扩展真实样本、附件和重收集 E3 | disabled |
| source.onenote | OneNote | Microsoft Graph OneNote API 加官方 Graph PowerShell SDK 薄调用 | 一次 Microsoft OAuth，最小 `Notes.Read` | E0：官方接口、权限和 SDK 路线已核 | 缺一次 OAuth 真实来源调用和 E3 | disabled |
...
| source.wechat_favorites | 微信收藏 | Windows 微信官方收藏 UI 加窄 UI Automation 适配器；按类型调用网页或媒体工具 | 已登录 PC 微信，打开收藏并选择可见集合或范围 | E0：官方 UI 与缺口已核 | 缺 Windows 窄 UI 适配器、真实样本和 E3 | disabled |
| source.wechat_articles | 微信公众号文章 | 已知链接用 OpenCLI `weixin download`；批量公众号用 `wechat-article-exporter` | 单篇给链接；批量历史需扫码登录自己的公众号后台 | E1：OpenCLI 和批量工具路线已核并调用工具控制面 | 缺 Bridge 或批量工具真实登录、附件和重收集 E3 | disabled |
...
| source.wechat_chats | 微信聊天记录 | 微信官方先迁移或备份到电脑；显式同意后用 `wechatDataBackup` 本地导出 | 同网手机确认或 PC 已有记录；明确选择会话和范围 | E1：候选工具与官方备份路线已核 | 缺版本兼容、法律安全确认、真实导出和 E3 | disabled |
| source.zhihu | 知乎收藏与内容 | `browser-use` 或 `agent-browser` 复用已登录 Chrome 发现并遍历收藏；OpenCLI 作第二层 | Chrome 已登录；首次批准本机远程调试；选择收藏范围 | E0：真实知乎来源未连接；通用浏览器连接证据另见工具表 | 缺知乎候选、正文、附件和重收集的真实探针 | disabled |
| source.bilibili | Bilibili 收藏与媒体 | 通用 Agent 浏览器列收藏和范围；`yt-dlp` 下载所选媒体，OpenCLI 补字幕和元数据 | Chrome 已登录；首次批准远程调试；选择收藏或视频范围 | E0：真实 Bilibili 来源未连接；浏览器和媒体工具证据另见工具表 | 缺 Bilibili 候选到媒体的 E3 闭环 | disabled |
| source.xiaohongshu | 小红书收藏 | 通用 Agent 浏览器发现收藏和详情；OpenCLI、MCP 或媒体工具作第二层 | Chrome 已登录；首次批准远程调试；选择收藏范围 | E0：真实小红书来源未连接；通用浏览器连接证据另见工具表 | 缺只读低频真实探针、附件和重收集 | disabled |
...
| source.browser_bookmarks | 浏览器书签 | 窄 Babata 扩展直接调用官方 `chrome.bookmarks` API | 安装扩展；按需授予 bookmarks；选择文件夹或集合 | E0：官方 API 与最小权限已核 | 缺 P4 扩展实现和真实 Chrome 样本 | disabled |
| source.browser_pages | 浏览器当前页面、选区和网页收藏 | 通用 Agent 浏览器导航读取；SingleFile 保存保真页面；长期入口再用窄扩展 | Chrome 原生远程调试一次批准；给出页面、站点或范围 | E0：真实页面来源未验证；通用浏览器连接证据另见工具表 | 缺真实页面探针、SingleFile 接入和重收集 | disabled |
| source.doubao | 豆包对话 | 通用 Agent 浏览器遍历历史；OpenCLI 稳定命令与会议 transcript 作第二层 | Chrome 已登录；给出会话、时间或数量范围 | E0：真实豆包来源未连接；浏览器和 OpenCLI 证据另见工具表 | 缺真实历史、正文、附件覆盖和重收集 | disabled |
| source.kimi | Kimi 对话 | 当前优先 Codex Chrome 调用 Kimi 结构化历史和会话接口；长期由 Skill 触发通用 Agent 浏览器 | Chrome 已登录；给出会话、时间或数量范围 | E2：真实历史分页、两条会话正文和外部 staging 已验证 | 缺全历史、附件或深研产物、逐条状态和重收集 | disabled |
| source.chatgpt | ChatGPT 对话 | 日常范围用通用 Agent 浏览器；OpenCLI 补结构化读取；账号级首次回收用官方 Data Export | Chrome 已登录；全量时只在 Data Controls 确认 | E0：真实 ChatGPT 来源未连接；浏览器和 OpenCLI 证据另见工具表 | 缺真实浏览器探针、附件覆盖和工作区资格核验 | disabled |
| source.local_files | 本地文件 | Babata 核心文件选择器、拖放或受控目录扫描直接读取 | 选择文件、目录或明确监视范围 | E0：核心路线和测试夹具已定义 | 缺 P3 唯一 C0 提交、回读和故障验证 | disabled |
| source.first_party | 第一方创作 | Babata 创作入口直接提交同一核心链路 | 明确执行新建、修订或批注 | E0：核心路线和测试夹具已定义 | 缺 P3 create、revise、annotate、关系与回读闭环 | disabled |
