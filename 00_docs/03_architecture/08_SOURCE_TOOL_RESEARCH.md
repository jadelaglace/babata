...
| source.browser_bookmarks | 浏览器书签 | 窄 Babata 扩展按需调用官方 `chrome.bookmarks` API，经受保护 loopback API 进入同一 Collector/C0 链路 | 安装扩展；按需授予 bookmarks；选择文件夹或集合 | E1：扩展已实现/构建，候选、显式选择、本机 API 和唯一 C0 writer 的隔离网络验证已通过 | 等用户在正式 Chrome 加载扩展后补真实书签文件夹样本；locator-only 书签不冒充网页正文 | disabled |
| source.browser_pages | 浏览器当前页面、选区和网页收藏 | Codex Chrome 负责存量导航；长期窄扩展用 `activeTab` + `scripting` 读取当前页/选区，SingleFile 后补保真页面 | 给出页面/站点范围；扩展安装后每次由当前页动作取得临时权限 | E1：扩展已实现/构建，页面/选区候选、显式选择、本机 API 和唯一 C0 writer 的隔离网络验证已通过 | 等用户在正式 Chrome 加载扩展后补真实页面/选区样本；SingleFile、附件和新鲜重采未覆盖 | disabled |
