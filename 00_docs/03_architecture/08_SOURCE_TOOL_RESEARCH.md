...
| source.wechat_favorites | 微信收藏 | 先实测 `wechat-cli favorites` 的本地只读候选；不兼容时再用官方收藏 UI 加窄 UI Automation | 已登录 PC 微信；CLI 初始化另需显式批准进程内存扫描和本地数据库解密 | E0：专用 CLI 的仓库、命令和 Windows 声明已核，尚未安装调用 | 缺当前微信版本兼容、权限安全确认、真实收藏和 E3 | disabled |
...
| source.wechat_chats | 微信聊天记录 | 微信官方先迁移或备份到电脑；显式同意后优先实测 `wechat-cli history/export`，不兼容再评估其他本地导出 | 同网手机确认或 PC 已有记录；明确选择会话和范围；CLI 初始化单独批准 | E0：专用 CLI 的本地 JSON/Markdown、时间范围和增量命令已核，尚未安装调用 | 缺当前微信版本兼容、法律安全确认、真实导出和 E3 | disabled |
...
2026-07-18 的补充搜索发现专用候选
[huohuoer/wechat-cli](https://github.com/huohuoer/wechat-cli)：仓库未归档，Apache-2.0，
README 提供 `favorites`、`sessions/history/search/export`、JSON 默认输出和时间范围；声明
支持 Windows 并在本机解密 SQLCipher 数据。它比先写 UI 适配器更接近 Agent 直接调用，
因此微信下一步必须先安装和实测该 CLI。

但这条路线不是无权限成本：`init` 会读取 `Weixin.exe` 进程内存提取密钥并保存本地配置；
当前仓库是 2026-04 创建的镜像，README 仍链接已不可访问的原仓库，Windows 兼容声明也
尚未在本机证明。运行 `init` 前必须单独说明并取得对进程内存扫描、本地数据库解密和明确
账号范围的批准；若版本不兼容或用户不批准，才回到官方收藏 UI 的窄适配。

此前调查没有找到一条在 Windows 上同时满足“仍维护、完整覆盖收藏类型、Agent 可调用、
无需解密私有数据库”的成熟工具。Mac 的
