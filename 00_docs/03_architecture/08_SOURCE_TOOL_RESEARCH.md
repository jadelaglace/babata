...
| 抖音收藏 | 路线已定 | `TikTokDownloader`/DouK-Downloader 读取收藏/收藏夹并增量下载 | 一次扫码或已登录浏览器 Cookie；用户选择收藏范围 | E1 |
...
- 与本项目直接相关的现成命令覆盖 Bilibili、知乎、小红书、ChatGPT、Kimi、豆包、
  公众号文章和通用网页；
- 本机 daemon 已运行，但 Browser Bridge 扩展尚未连接。`doctor` 明确报告 extension
  missing，因此当前只能证明命令和连接机制，不能宣称这些真实账号路线已通过；
...
OpenCLI 当前抖音适配器偏创作者后台，没有用户收藏命令。收藏主路线使用
[JoeanAmier/TikTokDownloader](https://github.com/JoeanAmier/TikTokDownloader)：GPL-3.0、
未归档，2026-07-14 仍有提交，支持抖音发布、喜欢、收藏、收藏夹、视频、图集、实况、
合集、评论，能保存 CSV/XLSX/SQLite，自动跳过已下载文件并增量运行。

最少用户动作：首次扫码或从已登录浏览器取得 Cookie；用户选择收藏/收藏夹、数量或
时间范围。凭据进入工具受控配置/secret store，不写日志和 Git。Agent 可调用终端模式或
Web API；`res-downloader` 只作为当前播放项的媒体捕获回退。

限制：非官方 API、Cookie 失效、验证码和风控；SQLite 是工具缓存，不是 Babata 权威；
下载权限和版权必须尊重平台和内容所有者。
...
研究路线已经确定，但除飞书外尚未达到 E3。下一步按价值和最小新增权限验证：
...
3. P4 浏览器扩展直接调用 `chrome.bookmarks`、activeTab 和 SingleFile，证明页面/选区/
   书签候选与选择性提交；
4. 语雀扩展验证知识库列表、目录、附件和一次重复导出；
5. OneNote 用 `Notes.Read` 验证一个 notebook/section/page/resource；Evernote 在用户明确
   账号级回收后验证一次 sync 和第二次增量 sync；
6. 微信按“公众号链接 -> 收藏 -> 视频号 -> 聊天”的风险顺序验证，数据库读取路线最后
   且必须再次明确确认；
7. 每条来源完成 E3 前保持 disabled，不用 fixture、命令 help 或下载文件数量冒充支持。

## 14. P2-G7 当前判断

所有 00 已点名来源都已经有明确的首选路线、最低用户授权、Agent 可做范围、限制和回退
决策；“先调研现有工具再设计”的文档缺口已补上。

但 P2-G7 原文还要求“实际验证”。当前只有飞书达到 E2，OpenCLI/其余工具多为 E1，
浏览器扩展、OneNote、微信窄适配仍为 E0/E1；尚未完成真实候选 -> 用户选择 -> 内容/
