...
1. **紧急存量回收**：优先用当前 Codex 已有的官方连接器/Skill、Codex Chrome 对正式版
   登录态的读取能力，以及只在桌面 UI 无结构化入口时使用 Computer Use。它们已经可用，
   不等待 Babata 长期自动化完成。
2. **脱离 Codex 的长期收集**：Browser Use 与 Agent Browser 均已证明可连接正式版 Chrome，
   后续由 Skill 触发；最终主次由长期真实来源 E3 证据决定，但不再阻塞紧急回收。
...
通用浏览器的长期连接验证已经足够，不再继续占用最高优先级。下一步改为：

1. 用 Codex 官方连接器/Skill 和正式版 Chrome 只读盘点当前已登录来源、存量范围、官方
   导出能力、附件与限制；
2. 按“官方 API/导出 -> Codex Chrome -> Computer Use -> 已验证站点工具”的顺序，为每个
   来源提出一次明确的批量下载/导出动作；涉及点击、下载、上传或修改前按用户授权边界确认；
3. 原始导出件和媒体先进入外部 `BABATA_DATA_HOME` recovery staging，保留来源、范围、
...
所有 00 已点名来源都已有第一轮逐站调查；Browser Use、Agent Browser、Playwright CLI
与 OpenCLI 的通用方向也已补查。Browser Use 与 Agent Browser 已连接同一个正式版 Chrome
并完成 tab/current-page 只读探针，因此“是否存在脱离 Codex 的浏览器工具”不再是阻塞。
抖音等具体来源仍缺 E3，这些缺口不能延迟当前 Codex 对存量资料的回收。

P2-G7 仍是**部分通过**：工具/连接实证已经补足，但多数具体来源尚未完成“明确范围 ->
候选/翻页/下载 -> 内容/附件 -> 重收集”的 E3 闭环，不能显示 available。P2 继续进行；
与此同时，按第 13 节启动 Codex 存量回收，不用长期自动化完成度阻塞资料抢救。
