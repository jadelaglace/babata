...
| source.onenote | OneNote | Windows 桌面 OneNote 官方 `.one/.onepkg` 备份或导出保留原件；`OneNoteMdExporter` 生成 Markdown/图片/附件可读副本；Graph 只作后续选择性发现和重采 | 桌面 OneNote 已登录且目标笔记本完整同步；每次选择笔记本/分区/页面范围；只有 Graph 路线需要一次最小 `Notes.Read` OAuth | E1：本机 OneNote/Word 16.0.20131.20126；官方 Graph 2.38.1；`OneNoteMdExporter` v1.6.0 发布包和 `--help` 已核 | 未读取真实笔记；缺官方原件导出、转换对照、C0、失败重试和 `unchanged` E3 | disabled |
...
| OneNote | 桌面 OneNote 登录并让目标笔记本完成同步；Graph 仅在后续选择性重采时由用户确认一次 `Notes.Read` | 全部笔记本、单个 notebook、section、page 或时间范围 | 官方 `.one/.onepkg` 原件导出、Markdown/附件副本、manifest/hash；需要时 Graph 分页和删除比对 | 选一个真实笔记本范围，执行原件+可读副本+C0+重采 E3 |
...
2026-07-19 复核后，正常路线改为先取得官方原件，再生成可读副本；Graph 不再冒充完整
备份。

第一层是 Windows 桌面 OneNote 的官方免费导出/备份：

- [Download a notebook](https://support.microsoft.com/en-us/office/download-a-notebook-516140ed-8df6-4943-a225-d9f6b1c124b5)
  明确支持桌面版按 notebook、section 或 page 导出，笔记本可保存为 OneNote Package
  (`.onepkg`)；PDF 只是查看副本，不是源文件；
- [Back up notes](https://support.microsoft.com/en-us/onenote/back-up-notes) 支持 Windows
  桌面版自动备份和 `Back Up All Notebooks Now`；官方恢复说明确认备份中的每个 `.one`
  文件代表一个 section；
- [How to transfer OneNote notebooks between accounts](https://support.microsoft.com/en-us/onenote/how-to-transfer-onenote-notebooks-between-accounts)
  说明工作/学校账号也可在 Windows 桌面版 `Save a Copy -> This PC`；
- OneNote Web 的整本导出只适用于个人 OneDrive，不适用于 OneDrive work/school 或
  SharePoint，因此不作为本机正常路线。

执行前必须让目标笔记本在桌面 OneNote 中完成同步。正式保存时 `.one/.onepkg` 是原件；
Markdown、DOCX、PDF 和页面截图都只能作为可读或恢复副本，不能替代原件。

第二层使用现有
[alxnbl/onenote-md-exporter](https://github.com/alxnbl/onenote-md-exporter/releases/tag/v1.6.0)
生成可读副本。v1.6.0（GPL-3.0，2025-12-15）仓库未归档，支持
`--all-notebooks --no-input`，也可限定 notebook、section 或 page；保留 section group/page
层级、created/updated 元数据、图片和文件附件。它离线调用已登录的 Windows OneNote 和
Word Interop，不需要额外云端凭据。

该工具是转换器，不是原件导出器：Markdown 不保留 section 内原始页面顺序，跨笔记本和
section 链接会丢失，绘图会扁平为图片，手写会丢失，密码分区必须预先解锁；仓库仍有图片、
附件引用和列表格式的开放问题。因此必须先保存官方原件，再核对笔记本/分区/页面/附件数量、
失败日志和抽样渲染，不能用“命令退出 0”代替内容验收。

2026-07-19 也复核了较新的替代项目：

- [dpokluda/onenote2md](https://github.com/dpokluda/onenote2md) 直接用 OneNote COM XML
  导出 section/page，提供 `--dry-run`、相对内部链接和字节级附件；但仓库 2026-06-23
  才创建，当前 0 star、无 release，必须自行构建 .NET 10，先观察而不进入正常路线；
- [rushleighconsulting/OneNote-Liberation](https://github.com/rushleighconsulting/OneNote-Liberation)
  提供 Graph HTML、附件、manifest、校验和续跑，但 2026-07 才创建、采用量极低且主要面向
  Apple Notes；当前由官方 Graph SDK 薄调用覆盖，不引入；
- [alopezrivera/OneNoteExporter](https://github.com/alopezrivera/OneNoteExporter) 与成熟候选
  使用相同 OneNote/Word/Pandoc 链路，但最新正式 release 仍是 2022 年的 1.0.0；
- [Obsidian Importer](https://github.com/obsidianmd/obsidian-importer) v1.8.12 有 OneNote
  导入能力，但输出绑定 Obsidian，属于下游迁移器，不作为 Babata 收集原件的入口。

第三层才是官方 Microsoft Graph OneNote API，适合选择性候选发现和后续重采：

- [Get OneNote content and structure](https://learn.microsoft.com/en-us/graph/onenote-get-content)
  可列 notebooks、section groups、sections 和 pages，并取得页面 HTML；默认每页 20 条、
  最大 100 条，账号内容多时应按 section 分页；
- 页面 HTML 中的原图和文件附件分别通过 resource URL 取得，但 API 不支持直接列出全部
  resources；
- 最小 delegated permission 为 `Notes.Read`，Graph OneNote 不支持 app-only；消费者账号、
  Microsoft 365 用户、group 和 SharePoint site 使用不同根路径；
- 官方明确提示 backup/restore notebook 或 section 会触及 OneNote API 限制。Graph 没有可
  依赖的完整 delta，重采应使用稳定 ID、`lastModifiedDateTime`、内容 hash 和全量 ID 差集；
- Graph 的输出 HTML 是官方 API 表示，不等同于 `.one` 原件。外部组织分享、21Vianet、
  历史版本、手写和复杂嵌入仍需真实样本验证。

本机证据：桌面 `ONENOTE.EXE` 和 `WINWORD.EXE` 均为 16.0.20131.20126；PowerShell Gallery
当前 `Microsoft.Graph.Authentication/Notes` 均为 2.38.1（2026-07-13 发布），本机版本一致；
v1.6.0 发布包 78,889,260 字节，SHA-256
`A7D4AEF91FD578C8BF8637AA15A8A184EF47B59402F3C2988D30CC5A3F0FF4A4`，本机只运行
`--help`，退出码 0，未连接 OneNote、未读取真实笔记。

决策：**官方桌面原件导出/备份 -> 现有 Markdown 导出器 -> Graph 选择性/增量补充**。
下一步不是开发适配器，而是由用户选一个真实笔记本范围；Agent 通过桌面窄 UI 完成同步
确认、官方原件导出和转换，Babata 负责 manifest/hash、C0、失败重试与 `unchanged` 重采。
...
| OneNote 只导 PDF/Markdown | 降级；必须先保留官方 `.one/.onepkg` 原件 |
| OneNote Graph 作为完整备份 | 降级；官方明确说明 notebook/section backup/restore 会触及 API 限制 |
