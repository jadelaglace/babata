...
类型、OneNote、印象笔记和书签自动遍历转入 P7 扩展；抖音和视频号按用户 2026-07-19
决定暂时不处理。通用机制可以复用，但未单独验证的平台始终保持 disabled。
...
策略。任何浏览器工具都不持有数据根路径、SQLite 凭据或最终资产权限。
...
微信公众号文章有真实小范围闭环；OneNote、印象笔记、微信聊天、视频号、抖音和书签自动
遍历等仍无对应真实闭环。所有未完整覆盖的来源继续 `disabled`；抖音和视频号明确延期，
不是 P4 阻塞项，也不计作完成证据。
