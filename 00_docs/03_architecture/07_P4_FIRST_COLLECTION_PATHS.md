...
优先使用用户授权的飞书官方 OpenAPI 或等价官方连接能力：

```text
本地配对/授权
  -> 浏览用户可见的文档、Wiki 和知识库层级
  -> 显示文档候选、类型、更新时间、附件/子节点可得性和权限限制
  -> 用户选择文档、当前层级可见集合或明确节点范围
  -> 按选择读取正文、原始/官方导出、附件和上下文
  -> CaptureService
```

实现必须处理 Wiki node 到实际文档的解析、分页、权限不足、已删除/移动、附件不可得
和速率限制。它不遍历用户未选择的整个账号，也不因获得 token 自动复制全部内容。
...
限制。扩展不持有数据根路径、SQLite 凭据或最终资产权限。
