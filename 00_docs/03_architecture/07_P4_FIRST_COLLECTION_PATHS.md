...
3. 浏览器当前页面、选区、链接和书签作为独立来源的候选与选择性收集。
...
结束后仍可调用的重试/重收集命令时，才调用 OpenCLI，并在证据中记录理由。长期浏览器
扩展负责更窄的当前上下文触发：书签直接用官方
...
小红书和语雀已分别由 Codex 正式 Chrome 到达官方站点，但当前登录态未建立：小红书显示
扫码/手机号登录遮罩，语雀显示“登录 / 注册”。两者均保留为等待用户完成官方登录的明确
阻塞；公开推荐流和语雀公开官网内容不计作用户收藏、赞过或知识库候选，也不写 C0。
...
| P4-G2 Kimi 与浏览器候选 | 正式 Chrome 中展示真实 Kimi 会话范围并收集所选消息；扩展配对另能展示页面/选区/书签候选。两者不能互相替代 |
