...
<!-- P5: in progress; TC-03A passed; job queue / AC-04 / TC-04 not closed; full AC-03/TC-03 awaits P6 TC-03B -->
...
- P5 已启动（进行中，未完成）：本机已验证百炼 CLI（`bl`）鉴权与多类型最小清洗试跑；引导型 Skill `02_skills/babata-bailian-clean/` 已接 `babata process register --pipeline agent_import` 正式写入 C1（staging ≠ 入库）。真实 PDF/图片/视频已绑定各自 C0 asset；旧占位结果已失效，视频原始 ASR JSON 与完整可读 transcript 已分开登记；C1 可审计删除重建、受限样本、first-party annotation 和 DOCX 原件/PDF 预览边界已完成，TC-03A 通过。Provider 作业队列与 AC-04/TC-04 尚未关闭；完整 AC-03/TC-03 仍需 P6 TC-03B。清洗派生物与 C1 只进 `BABATA_DATA_HOME`，不进 Git。
...
6. AC-03 的 P5 C0/C1 子责任与 TC-03A 已通过；Provider 作业队列（enqueue/run-once）、
   AC-04 与 TC-04 仍未关闭；完整 AC-03/TC-03 还包含 P6 的 TC-03B。
7. Issue #48 已补上媒体 kind 强制 asset、run target kind/asset 身份、受控 C1 文件 staging
   ...
   删除重建、受限样本和 DOCX 原件/PDF 预览证据均已完成。TC-04 与 job queue 继续由 Issue
   #46 跟踪，其他 C0 字段审计由 Issue #43 跟踪。
