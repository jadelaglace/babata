...
prepared/C0-B 与 registered/C0-C 的共同准入门槛是 A2；A1-only 不进入 B/C，A3+ 不阻塞 B/C。

按这一口径，当前微信文件传输助手与收藏第一阶段只达到 A1：记录、URL 和本地可得资源已
保全，但大量 URL 正文/媒体及缺失必要引用尚未形成 A2 完整性闭环。豆包 MBA 第一阶段已按
完整消息链和声明附件逐项覆盖，达到 A2。二者都先按真实主权深度汇报，不抢跑 B/C。

P7 的微信正式能力证明只取 10–20 条有界代表样本，文件传输助手和收藏两类都覆盖，优先选择
URL/文章并带少量本地媒体。Agent 必须逐条取得正文、内嵌媒体和必要附件达到 A2，随后走
prepared/C0-B、registered/C0-C 和 unchanged 重采；其余第一阶段全量继续保持 A1 Recovery。
该小样本足以验证新来源/内容形态与受控 Agent，不把全量微信 A2 错列为 P7 阶段门槛。
