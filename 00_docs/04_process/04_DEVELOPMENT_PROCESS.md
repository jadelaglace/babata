...
<!-- P5: in progress; TC-03A passed; job queue implemented, real TC-04 / AC-04 not closed; full AC-03/TC-03 awaits P6 TC-03B -->
...
- P5 已启动（进行中，未完成）：本机已验证百炼 CLI（`bl`）鉴权与多类型最小清洗试跑；引导型 Skill `02_skills/babata-bailian-clean/` 已接 `babata process register --pipeline agent_import` 正式写入 C1（staging ≠ 入库）。真实 PDF/图片/视频已绑定各自 C0 asset；旧占位结果已失效，视频原始 ASR JSON 与完整可读 transcript 已分开登记；C1 可审计删除重建、受限样本、first-party annotation 和 DOCX 原件/PDF 预览边界已完成，TC-03A 通过。Provider 作业队列、local extract 和 live Bailian summary 已实现并通过 fixture；真实 TC-04 尚未执行，因此 AC-04 仍未关闭。完整 AC-03/TC-03 仍需 P6 TC-03B。清洗派生物与 C1 只进 `BABATA_DATA_HOME`，不进 Git。
...
P4 已收尾，下一阶段
  P5         C1 多模态清洗与百炼处理（进行中：Skill→register 和 job 队列已接通，真实 TC-04 待执行）
...
6. AC-03 的 P5 C0/C1 子责任与 TC-03A 已通过；Provider 作业队列的
   `enqueue/run-once/status/retry/cancel`、本地 UTF-8 asset 提取和百炼文本摘要已实现并通过
   fixture 生命周期测试，但真实百炼失败/重试证据尚未执行，因此 AC-04 与 TC-04 仍未关闭；
   完整 AC-03/TC-03 还包含 P6 的 TC-03B。
7. Issue #48 已补上媒体 kind 强制 asset、run target kind/asset 身份、受控 C1 文件 staging
   ...
   禁止规则。job queue 实现与 TC-04 继续由 Issue #46 跟踪，其他 C0 字段审计由 Issue #43 跟踪。
...
2. ~~Agent 清洗 Skill 与 `process register` 接通；job 队列及 local extract/live Bailian summary provider 实现~~；真实 TC-04 执行仍待；
3. ~~真实绑定、受控文件、输出一致性、retry 身份、真实样本、删除重建与 TC-03A~~；
4. 文档/网页机械提取进入 Babata process/provider；
5. 图片 OCR、音频转写、视频字幕/关键帧/视觉描述的启用矩阵与不可用声明；
6. 百炼/通义 API 或批处理、隐私、成本、限流和重试；
7. 原件/派生物对照与转换损失说明；
8. ~~通过 AC-03 的 P5 C0/C1 子责任与 TC-03A~~；继续通过 AC-04 与 TC-04。AC-03 整体和
   TC-03 整体待 P6 的 TC-03B 通过后关闭。
