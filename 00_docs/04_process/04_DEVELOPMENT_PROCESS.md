...
P8  全账户回收闭合、备份恢复与长期加固              进行中（P8.1 已完成）
```
...
### 11.1 P8.1：一致快照、加密备份与隔离恢复

当前只执行 P8.1。活动 SQLite 必须使用一致快照机制，不能把运行中的数据库文件直接复制后
冒充备份；快照冻结 C0/C1 manifest 和哈希后交给 Git 外加密增量备份。至少连续建立两次
快照证明未变化内容复用，再恢复到全新隔离数据根，验证数据库、外键、manifest、C0 资产和
可重建层状态。隔离根不自动替换活动根；密码、备份仓库和真实报告均留在 Git 外。

2026-07-27，Issue #112 完成 P8.1。官方 restic 0.19.1 Windows 便携包经发布 SHA-256
`da948ad707ed690426473aaba2046cd61f8f90f6f0e7dab6be0d5796531de67d` 校验后放在 Recovery
工具根；随机密码文件、加密仓库、snapshot state 和隔离恢复均在 Git 外。两次活动根一致快照
各覆盖 1,281 个文件、21,310,423,952 字节；第一次新增 5,764,207,050 字节，第二次只新增
2,467,774 字节。第二次 snapshot 在隔离根 1,281/1,281 文件全量 hash、4 个 SQLite quick
check/外键均通过；一个真实 C0 资产副本被故意篡改后验证 fail closed，恢复原字节后再次全量
通过。活动根异常计数始终为 0。证据位于
`BABATA_EVIDENCE_HOME/runs/p8-1-backup-20260727-214924/`。至此 P8.1、AC-10、TC-10 完成；
P8.2/P8.3 尚未启动，P8 整体继续进行中。
...
P8.3 才推进其余点名来源的全量主权回收，目标是每个计划内来源的全量范围至少达到 C0-A1；
...
registered/C0-C。P8.3 不在 P8.1 期间提前启动。

P8.1 已完成 AC-10、TC-10；P8.2/P8.3 完成后，P8 再与 P4–P7 的真实路径共同完成
AC-11、TC-11 的完整本地 raw-to-view 闭环。
...
| P8 | AC-10（P8.1 已通过）、AC-11 | 一致恢复已完成；全账户回收与完整系统闭环继续进行 |
