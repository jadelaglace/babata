...
- 日常 Agent 循环使用 `05_scripts/check-fast.ps1`，必要时通过 `-RustPackage` 只检查和测试
  受影响 package；架构、writer、数据根或文档权威边界变化使用 `check-boundary.ps1`；合并前
  使用 `check-full.ps1`。三个入口是本地与 CI 的共同命令，并输出分组耗时；
