...
P8  来源广度、豆包/微信后续阶段与全量 A1             进行中（群聊暂缓；P8.3 已启动并保留真实缺口）
...
| Bilibili | 33 个收藏夹全部收藏关系 | 3,167 条收藏关系、3,116 个唯一 URL | 部分完成 | 18 个收藏夹声明 3,228、实得 3,167，共差 61；同类问题超过 3 个对象后暂停 | 登录态 OpenCLI 手动分页 |
...
| 浏览器书签/网页 | 当前浏览器用户给定书签/网页范围 | P8.1 已有 5 条 | 阻塞 | Chrome 安全策略禁止 Agent 读取 `chrome://bookmarks/`，全量分母未知；需用户导出书签 HTML | 用户导出 HTML 后本地只读解析 |
...
抖音和视频号继续为 non-plan，不计入完成率。Bilibili、知乎和小红书的差额均有可读收藏夹、
标题或页面证据，不只保存 hash/ID；飞书 Drive 与浏览器书签也保留失败文件和正常恢复路线。
因此 P8.3 仍进行中，不能把已完成来源或当前可见分页冒充全来源完成。

P8.2/P8.3 完成后，P8 再与 P4–P7 的真实路径共同完成
...
| P8 | AC-09（P8.1 最低覆盖）、AC-11 | P8.1 已完成 15/15；P8.2 中豆包第二/第三阶段、微信第二阶段和第三阶段其他范围已完成，132 个群聊按用户决定暂缓；P8.3 继续保留其全量 A1 真实缺口；P8.4 已完成 14/14 纳入来源、70 条 C0-A2 样本，Bilibili 按用户决定整体暂缓且不计分母 |
