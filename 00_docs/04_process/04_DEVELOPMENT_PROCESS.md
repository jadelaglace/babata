...
<!-- P5: in progress; Bailian CLI proven + agent skill landed; AC-03/AC-04 and TC-03/TC-04 not yet closed; C1 core registration pending -->
...
- P5 已启动（进行中，未完成）：本机已验证百炼 CLI（`bl`）鉴权与多类型最小清洗试跑（图 OCR、文档结构化、表格/课件摘要、视频 ASR 时间戳/说话人分句）；引导型 Skill 已入库 `02_skills/babata-bailian-clean/`。C1 schema/process run 正式登记、失败重试并存记录、以及 AC-03/AC-04 与 TC-03/TC-04 尚未关闭。清洗派生物只进 `BABATA_DATA_HOME`，不进 Git。
...
P4 已收尾，下一阶段
  P5         C1 多模态清洗与百炼处理（进行中：Skill 已入库，核心登记未完成）
...
### 8.1 已证明（仓库外试跑 + 已入库 Skill + C1 登记底座）

1. 百炼 CLI（`bl`）可安装、鉴权，并作为首个多模态处理路径使用。
2. 对本机课程样例做了每类型最小验证：图 OCR、PDF/DOCX/XLSX/PPTX 结构化摘要、视频截帧与 ASR 转写（含时间戳；说话人字段在单讲师样本中可见）。
3. Agent 引导 Skill 已入库：`02_skills/babata-bailian-clean/`（原件只读、本地规范化、百炼路由、派生物契约）。
4. C1 最小登记路径已激活：`derived.sqlite`（`process_runs`/`derivatives`）、`babata process list-pipelines|register|show-run|list-runs`；重试新建 run，不覆盖旧结果。
5. Provider 作业队列（enqueue/run-once）与 AC-03/AC-04、TC-03/TC-04 仍未关闭。

### 8.2 仍待完成（关闭 P5 前）

依次激活：

1. ~~C1 schema、process run 和 derivative 的最小登记路径~~（已完成 register）；完整 job 队列与 provider 执行仍待；
2. 文档/网页机械提取进入 Babata process/provider；
3. 百炼 CLI 从 Agent 试跑收敛为可追溯 pipeline/run 记录；
4. 图片 OCR、音频转写、视频字幕/关键帧/视觉描述的启用矩阵与不可用声明；
5. 百炼/通义 API 或批处理、隐私、成本、限流和重试；
6. 原件/派生物对照与转换损失说明；
7. 通过 AC-03、AC-04 与 TC-03、TC-04。
