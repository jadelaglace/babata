...
<!-- P6.2 preflight: Issue #60 common C0 metadata/observations implemented; AC-07/TC-07 not started -->
...
- Issue #60 已完成 P6.2 preflight 实现：新增 `babata.c0.common/v1`、
  ...
  这只为后续检索提供稳定输入，不代表 P6.2、AC-07 或 TC-07 已开始或通过。
...
本切片不创建搜索 projection，不实现多条件检索、评分排序、关系导航、子库或通用输出；
AC-07/TC-07 仍保持未开始。正式 P6.2 必须另开 Issue。
