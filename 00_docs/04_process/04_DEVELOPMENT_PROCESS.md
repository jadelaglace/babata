...
6. Provider 作业队列（enqueue/run-once）与 AC-03/AC-04、TC-03/TC-04 仍未关闭。
7. 本轮以“百炼 CLI 跑通 + Skill→register 最小接线”为边界收尾；真实 C0→C1 强校验、失败重试和验收加固转 Issue #48，job queue 继续由 Issue #46 跟踪，C0 字段审计由 Issue #43 跟踪。
...
3. 文档/网页机械提取进入 Babata process/provider；
