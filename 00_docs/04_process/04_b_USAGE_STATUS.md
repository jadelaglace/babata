...
| P8.9 | 进行中 | 财务管理、供应链和决策会计已关闭；执行商务沟通已发布并待全课程统一验收 | 已关闭两课的宇宙归属仍是历史单路径模型；map-node 非父边类型化关系、course/branch 分离、多重归属、MBA lens、基石强度/置信度、兼容迁移、其余课程交付、全课程统一验收及适用的 closure 尚未完成 |
