...
14. 模拟平台将在一小时内不可访问，同时让解析、A2 递归和 Rust 登记不可用；确认 Agent
    仍先完成 A1，把直接可得字节/响应/导出、来源、哈希和限制保存到本地，并准确报告为
    captured 而非正式 C0。
15. 对一篇正文含内嵌图片、必要附件和外部引用文章的页面执行取得；确认正文、内嵌图片和
    必要附件属于 A2，取得失败时逐项记缺口；外部引用默认只记 reference，只有显式授权后
    才以有界 A3+ 形成独立 node/reference，且不会继续无限追取下一层引用。
...
8. 分别对 A1-only 和 A2-complete 的 captured 上游归档请求解压、稳定命名、格式识别和
   manifest 生成；确认前者被 B/C 准入门拒绝，后者可形成 prepared/C0-B。逐文件比较前后
   字节与 hash，确认准备只新增结果，不覆盖、移动或规范化改写原件。
9. 分别提供 A2+prepared 和 registered 样本；确认只有后者经过唯一 Rust writer，具有
...
A1/A2/A3+ 与 captured/prepared 场景留待相应取得与报告实现时补充执行证据。
...
11. 对已经正式取回的“战略领导力W1”只建立全新临时数据根：从 Chrome 官方响应生成包含
    ...
    其他值时，确认 C0 写入前失败。
12. 对同一批结果分别制造 `A1 + captured`、`A2 + prepared` 和 `A2 + registered`；确认 Skill
    汇报同时显示主权深度与管理状态，并只把最后一种称为正式 C0。再从 C2 提交一个 A3
    缺口请求，确认它走新的来源授权与取得任务，不通过 Skill 或 C2 直接改写旧 C0。
13. 用只含收藏 URL/本地资源的微信 A1 范围和消息/声明附件完整覆盖的豆包 A2 范围对照；
    确认微信范围在取得 URL 正文、内嵌媒体和必要附件前不能进入 B/C，豆包范围可继续准备，
    A3 语义引用是否取得不影响两者的 B/C 准入判断。
14. 从微信文件传输助手和收藏中选择 10–20 条代表样本，两类都覆盖，优先覆盖 URL/文章并
    包含少量本地媒体；逐条取得正文、内嵌媒体和必要附件，确认样本达到 A2 后进入 prepared/C0-B、registered/
    C0-C，并在重采时正确报告 unchanged。核对未选全量仍为 A1，汇报不扩大样本结论。
