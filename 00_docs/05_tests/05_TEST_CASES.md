...
场景：用户审阅一条外部资料及其 C1，在个人知识宇宙中建立地图/方向、知识、案例、
日志和感悟，完成关系、三维评分和高密度表达；模型同时提出已审阅和未审阅建议。

步骤：

1. 在同一审阅上下文查看原件、派生物、来源、版本和关系。
2. 创建时间/空间/物质/意识四根及至少一个学科、分支；把同一内容归属多个节点，修订
   一个学科/分支或归属并回看旧地图版本。
3. 各创建或修订一项 Knowledge、Case、Log 和 Insight；Log 覆盖至少两种时间尺度，
   Knowledge 与 Case 双向互证，Insight 引用一项 Knowledge 或 Log。
...
6. 保留一个模型建议为 unreviewed，同时分别接受、修改和拒绝其他建议；继续执行检索、
   关系导航、子库候选和输出候选。
7. 搜索和回看人工记录、旧版本、原建议和审阅标记，检查与原始证据冲突的模型建议。

预期：三级地图、多重归属、五类语义、关系、评分/profile 和高密度文本均可追溯；人工
成果拥有作者、时间、目标、依据和版本；模型建议始终为 C1；审阅只追加状态标记，修改
产生新内容时才创建 first-party C0；拒绝不删除建议；unreviewed 不阻塞下游且不冒充人工；
删除图片/视图不损伤文本表达；任何建议都不覆盖原件或唯一分类。
...
2026-07-20，Issue #59 首切片在临时数据根通过 Knowledge create/revise/show 两版回读，
并用真实微信 C0/C1 通过 `knowledge review` 的跨库引用与 active hash 校验。该证据只覆盖
TC-05/TC-06 的入口和版本机制；尚未形成用户明确判断的真实 Knowledge，也未覆盖地图、
五类语义、评分/profile、建议审阅和视图删除重建，因此 TC-05、TC-06 均未整体通过。
