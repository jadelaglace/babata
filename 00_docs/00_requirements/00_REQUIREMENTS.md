...
正常情况下，我只需要做来源本身无法替代的最小动作：在官方页面登录或确认 OAuth、扫码、批准必要的本机/浏览器权限，再选择单条、可见集合或明确范围。工具安装、命令调用、分页、下载、重试、临时目录、格式接入、状态和溯源由 Agent/Babata 处理。不要把密码、Cookie、token、导出路径或元数据手抄给 Agent 当成日常流程；只有某个现有工具被真实证明没有更安全的授权方式时，才把受控秘密导入作为明确限制和最后选择。
