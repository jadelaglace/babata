...
10. 当来源存在短期消失风险时，A1 能在解析、直接依赖递归和正式登记均不可用的情况下先
    保存直接可得字节/响应/导出、来源身份、哈希和限制；产品不会为了等待 A2 或 registered
    C0 而丢失这批已可取得资料。
11. A2 只追取使当前内容本地完整所必需的正文、内嵌媒体和必要附件，并逐项显示已取得或
    缺失；被正文引用的文章、论文和页面默认保持 reference，只有明确授权且有价值时才进入
    有界 A3+，每个取得结果形成独立 node/reference，不发生无限递归。
...
7. 一个明确来源范围在原件、上下文和限制进入统一 C0 后即可独立显示收集完成，零项 C1
   ...
   一条平台专属 C1 链路。
8. 只有正文、内嵌媒体和必要附件等 A2 完整性依赖均已取得或明确证明不适用后，captured
   原件才可经过解压、稳定命名、格式识别、manifest 或字段保留成为 prepared/C0-B；上游原件
   仍保持字节一致且可独立校验。A1 结果即使已在本地保全，也不得直接进入 B/C。
9. 只有统一 Rust application/core 成功写入并回读 item、revision、asset、provenance、relation
   ...
   已拿回及尚未登记。主权深度 A1/A2/A3+ 与管理就绪状态分别可见。
...
10. 总 Skill 对每项结果同时报告主权深度 A1/A2/A3+ 和管理就绪 captured/prepared/registered；
    A1/A2 可以在尚未 registered 时作为成功主权回收报告，但不得称为正式 C0。A3+ 必须有
    显式价值、授权和停止边界；C1/C2 只能提出新取得请求，不能经外围入口反写旧 C0。
11. prepared/C0-B 和 registered/C0-C 都拒绝 A1-only 输入；A2 是两者共同准入门槛，A3+
    完成度不影响准入。含未取得 URL 正文、内嵌媒体或必要附件的范围继续报告为 A1/缺口。
12. 微信 P7 从文件传输助手和收藏中选择 10–20 条代表样本，两类都覆盖。样本逐项取得 URL
    正文、内嵌媒体和必要附件达到 A2 后，才能进入 prepared/registered 并验证重采；范围外
    的第一阶段全量仍显示 A1，不因小样本通过而显示为全量 A2、全量正式登记或全来源支持。
